...
ZeroAudioEngine::ZeroAudioEngine(Link& link)
  : mLink(link)
  //, mSampleRate(44100.)
  , mOutputLatency(0)
...
void ZeroAudioEngine::setTempo(double tempo)
{
  std::lock_guard<std::mutex> lock(mEngineDataGuard);
  mSharedEngineData.requestedTempo = tempo;
}

double ZeroAudioEngine::quantum() const
{
  return mSharedEngineData.quantum;
}

void ZeroAudioEngine::setQuantum(double quantum)
{
  std::lock_guard<std::mutex> lock(mEngineDataGuard);
  mSharedEngineData.quantum = quantum;
}

bool ZeroAudioEngine::isStartStopSyncEnabled() const
{
  return mLink.isStartStopSyncEnabled();
}

void ZeroAudioEngine::setStartStopSyncEnabled(const bool enabled)
{
  mLink.enableStartStopSync(enabled);
}


void ZeroAudioEngine::setBufferSize(std::size_t size)
{
  mBuffer = std::vector<double>(size, 0.);
}

void ZeroAudioEngine::setSampleRate(double sampleRate)
{
  mSampleRate = sampleRate;
}

ZeroAudioEngine::EngineData ZeroAudioEngine::pullEngineData()
{
...
/*
void ZeroAudioEngine::renderMetronomeIntoBuffer(const Link::SessionState sessionState,
...
void ZeroAudioEngine::audioCallback(
